...
/* ضع رابط السيرفر الخلفي بتاعك هنا بعد ما تعمله deploy (شوف مجلد backend/)
   مثال: 'https://download-backend.onrender.com'
   لو سبته فاضي أو السيرفر مش شغال، الموقع هيرجع تلقائي للنسخة التجريبية */
const API_BASE = '';

async function tryRealBackend(realUrl){
  if(!API_BASE || !realUrl) return null;
  const endpoint = `${API_BASE.replace(/\/$/,'')}/api/download?url=${encodeURIComponent(realUrl)}&quality=${currentOptId}`;
...
async function startDownload(){
  ...
  // 1) جرّب السيرفر الحقيقي الأول (لو متوصّل)
  if(API_BASE){
    try{
      label.textContent = 'جاري التحميل من السيرفر… ';
      fill.style.width = '35%';
      const blob = await tryRealBackend(realUrl);
      fill.style.width = '100%';
      label.textContent = 'تم ✓';
      const ext = currentOptId === 'audio' ? 'mp3' : 'mp4';
      triggerDownload(blob, `download.${ext}`);
      document.getElementById('successSub').textContent = 'الملف الحقيقي اتحفظ على جهازك';
      document.getElementById('successBox').classList.add('show');
      btn.disabled = false;
      return;
    }catch(err){
      console.warn('السيرفر الخلفي مش متاح، هنرجع للنسخة التجريبية:', err);
      fill.style.width = '0%';
    }
  }

  // 2) نسخة تجريبية (fallback) — بتولّد ملف حقيقي قابل للتشغيل لكنه مش الفيديو الأصلي
  try{
...
/* ==================================================================
   6) Cursor-reactive icons — orbs + orbit platform icons + grid icons
   follow the mouse/touch with a smooth parallax feel
   ================================================================== */
(function initCursorReact(){
  const orbs = document.querySelectorAll('.orb');
  const orbitWrap = document.querySelector('.orbit-wrap');
  const cardIcons = document.querySelectorAll('.p-card .icon-wrap');
  const navLogo = document.querySelector('header .logo .ring');
  ...
  function loop(){
    mx += (tx-mx)*0.07;
    my += (ty-my)*0.07;
    const dx = (mx-innerWidth/2)/(innerWidth/2);
    const dy = (my-innerHeight/2)/(innerHeight/2);
    ...
    if(orbitWrap) orbitWrap.style.transform = `translate(${dx*14}px, ${dy*14}px) rotate(${dx*1.5}deg)`;

    cardIcons.forEach((ic,i)=>{
      const depth = 3 + (i%3)*2;
      ic.style.transform = `translate(${dx*depth}px, ${dy*depth}px)`;
    });

    if(navLogo) navLogo.style.transform = `translate(${dx*3}px, ${dy*3}px) rotate(${dx*6}deg)`;
...
/* ==================================================================
   7) Scroll reveal — steps appear progressively as you scroll
   ================================================================== */
